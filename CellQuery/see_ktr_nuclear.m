...
addParameter(p,'StartThresh',800, valid_conv); %min allowable starting threshhold to filter out cells with low starting KTR_nuc (pre-activated or just heterogeneity?) KTR, default is 200
% addParameter (p, 'Baseline', 1, @isnumeric); %??leave this out for KTR for now, maybe set to 0.2 if needed
addParameter (p, 'GraphLimits',[0 400],@isnumeric);
