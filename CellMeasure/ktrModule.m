...
%
%
% SL: adjust KTR module to use annulus (small ring around nucleus) to estimte
...
% Use annulus to estimate cytoplasmic area.
% by dilating nucleus by a factor of the minimum nuclear radius given in
% parameters and using C++ function to generate cell labels
...
cyto_cc = cell_cc;
for i = 1:min([length(cyto_cc.PixelIdxList),length(nuc_cc.PixelIdxList)]) % this loop removes from cyto the pixel that are also part of nucleus
    ...
    if ~isempty(AuxImages{i})
        ...
        for n = 1:cyto_cc.NumObjects        
            if ~isempty(nuc_cc.PixelIdxList{n})
                ...
                median_n(median_n==0) = NaN; % change 0 to NaN
                median_c = median(AuxImages{i}(cyto_cc.PixelIdxList{n}));
                median_c(median_c==0) = NaN; %  change 0 to NaN
                % Assign measurements
