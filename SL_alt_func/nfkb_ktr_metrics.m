function [metrics,aux, graph, info, measure] = nfkb_ktr_metrics(id,varargin)
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
...
% Optional parameters
addParameter (p, 'BaselineNFkB', 1, @isnumeric);
addParameter (p, 'BaselineKTR', 0.1, @isnumeric);
addParameter(p, 'StartThreshNFkB', 2, @isnumeric); 
addParameter(p,'StartThreshKTR',0.6, @isnumeric);
...
BaselineNFkB = p.Results.BaselineNFkB; % Minimum activity required for cell to register as 'on'
BaselineKTR = p.Results.BaselineKTR; % Minimum activity required for cell to register as 'on'
...
[graph, info, measure] = filter_nfkb_ktr(id,'MinLifetime',MinLifetime,...
                            'ConvectionShift',ConvectionShift, 'BaselineNFkB',BaselineNFkB,...
                            'BaselineKTR',BaselineKTR,'MinSize', MinSize,'StartThreshNFkB',...
                            StartThreshNFkB,'StartThreshKTR', StartThreshKTR, 'Verbose', p.Results.Verbose,...
                            'GraphLimitsNFkB', p.Results.GraphLimitsNFkB, 'GraphLimitsKTR', p.Results.GraphLimitsKTR);
...
if ~ismember ('BaselineNFkB',p.UsingDefaults)
    BaselineNFkB = info.BaselineNFkB;
end
if ~ismember ('BaselineKTR',p.UsingDefaults)
    BaselineKTR = info.BaselineKTR;
end
...
inactive = smoothrows(inactive<(BaselineNFkB),(window_sz*2));
...
aux.thresholds = linspace(0, BaselineNFkB*3, 40);
...
inactive = [repmat(nanmin(smoothed(:,1:7),[],2),1,window_sz*2+1),smoothed(:,:),...
    repmat(nanmedian(smoothed(:,(end-window_sz:end)),2),1,window_sz*2)];
inactive = smoothrows(inactive<(BaselineKTR),(window_sz*2));
frontcrop = round(window_sz*2*(1-thresh))+window_sz+1;
inactive = inactive(:,frontcrop:end);
inactive = inactive(:,1:size(smoothed,2));
inactive(isnan(smoothed)) = nan;
...
%aux.fft = zeros(size(round(metrics.time_series_ktr,1)),NFFT/2+1);
...
aux.thresholds = linspace(0, BaselineKTR*3, 40);
