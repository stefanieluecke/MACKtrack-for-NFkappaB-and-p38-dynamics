function [graph, info, measure] = see_ktr_cytoplasmic(id,varargin)
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
% [graph, info, measure] = see_ktr_cytoplasmic(id,graph_flag, verbose_flag)
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
% see_ktr_cytoplasmic is a data processing.and visualization script currently being derived from see_nfkb_native (specialized to handle
% a nuclear-translocating species). It looks for KTR_cyto1 measurements).
%
% INPUTS (required):
...
%                   with pre-activated ktr, default is 500 %tbd whehter this is useful for ktr function
% 'GraphLimits'     default is [0 100]
...
addParameter(p,'StartThresh',500, valid_conv); %max allowable starting threshhold to filter out cells with high (pre-activated or just heterogeneity?) KTR cyto, default is 500
% addParameter (p, 'Baseline', 1, @isnumeric); %??leave this out for KTR for now, maybe set to 0.2 if needed
addParameter (p, 'GraphLimits',[0 100],@isnumeric);
...
ktr = measure.KTR_cyto1(:,:); %ktr is defined as KTR cytoplasmic/nuclear ratio from ktr module
