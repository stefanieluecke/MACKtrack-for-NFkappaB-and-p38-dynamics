function [] = see_NFkB_KTR_nuc_HM_sorted_test(id, varargin)
...
parse(p,id, varargin{:})
%%
[metrics,aux, graph, info, measure] = nfkb_ktr_nuc_metrics(id, 'MinLifetime',p.Results.MinLifetime,...
...
colormapStack_inv_color(graph.var_ktr(graph.order,:),graph.celldata(graph.order,:), graph.opt_ktr); %calls subfunction that makes figure    
