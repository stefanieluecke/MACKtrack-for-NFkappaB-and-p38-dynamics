function [graph, info, measure] = filter_nfkb_ktr(id,varargin)
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
...
%test 20200227
nfkb = nfkb/mean(info.parameters.adj_distr_NFkBdim(2,:));
%nfkb = nfkb/mean(info.parameters.adj_distr_NFkB(2,:)); %nfkb is re-defined of nfkb divided by mean of second row of adj_distr %? but I don't know what that does...
...
% !!! test 101619 somehow, the use of ratio causes problems in ktr metrics
% part of nfkb_ktr_metrics in calculating peak freq in some, but not all
% conditions; ONLY when using see_NFkB_KTR_HM_sorted_test, not when using metrics or original see functions directly... 
%?? don't know why or how to properly solve problem
%ktr = round(100.*measure.KTR_ratio1(:,:)); %ktr is defined as KTR cytoplasmic/nuclear ratio from ktr module
...
if verbose_flag
    filter_str = {'didn''t exist @ start', 'short-lived cells', 'very dim NFkB',...
        'NFkB outliers [mean val >3*std]','extreme KTR val [mean>1.7*std]', 'KTR outliers [mean val >3*std]','extreme NFkB val [mean>1.7*std]', 'NFkB active @ start','KTR active @ start', 'high nuclear stain','low area'};
