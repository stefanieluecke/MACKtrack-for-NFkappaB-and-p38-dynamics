function [graph, info, measure] = see_ktr_nuclear_testing(id,varargin)
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
...
addParameter(p,'StartThresh',1000, valid_conv); %min allowable starting threshhold to filter out cells with low starting KTR_nuc (pre-activated or just heterogeneity?) KTR, default is 200
...
%test 09/27/19 do not remove baseline from nuclear
...
%droprows =  [droprows, prctile(ktr(:,1:8),18.75,2) < start_thresh]; %removes cells for which the 19th percentile of the first 8 timepoints is smaller than start threshold %/why?
