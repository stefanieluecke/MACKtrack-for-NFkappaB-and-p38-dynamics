function [graph, info, measure] = see_ktr_SL(id,varargin)
%- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
...
% % 'Baseline'        default is 1 in nfkb function %? baseline seems to be defined without this below. %?Is this used at all..? %does baseline defining make sense for ratios like this?
        %seems like Baseline is used in metrics function, define there
...
info.GraphLimits = p.Results.GraphLimits; % Min/max used in graphing derived from input parameters
...
%test 07-12-19 remove smoothing before baseline calculation, since this
%introduces 0 in first position, perhaps resulting in nothing being
%deducted as baseline? --> no, put it back in (19/09/06)
...
%ktr_min = prctile(ktr(:,1:baseline_length),5,2); %calculates the 5th percentile along rows of the ktr smoothed trajectory up to the baseline length, Ade's nfkb version uses 2,2 instead of 5,2
