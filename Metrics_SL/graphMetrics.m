...
switch p.Results.FilterResponders 
    ...
    case {'nfkb', 'respective'}
       for i = 1:n
        for k = 1:numel(viol_met_nfkb)
            if i==1 
            violin.(viol_met_nfkb{k}) = {ID(i).metrics.(viol_met_nfkb{k})(ID(i).metrics.responder_index_nfkb == 1)};
            else
            violin.(viol_met_nfkb{k}) = [violin.(viol_met_nfkb{k}), ID(i).metrics.(viol_met_nfkb{k})(ID(i).metrics.responder_index_nfkb == 1)];
            end
        end
       end
    case 'ktr'
       for i = 1:n
        for k = 1:numel(viol_met_nfkb)
            if i==1 
            violin.(viol_met_nfkb{k}) = {ID(i).metrics.(viol_met_nfkb{k})(ID(i).metrics.responder_index_ktr == 1)};
            else
            violin.(viol_met_nfkb{k}) = [violin.(viol_met_nfkb{k}), ID(i).metrics.(viol_met_nfkb{k})(ID(i).metrics.responder_index_ktr == 1)];
            end
        end
       end
    case 'both'
       for i = 1:n
        for k = 1:numel(viol_met_nfkb)
            if i==1 
            violin.(viol_met_nfkb{k}) = {ID(i).metrics.(viol_met_nfkb{k})(ID(i).metrics.responder_index_nfkb == 1 & ID(i).metrics.responder_index_ktr == 1 )};
            else
            violin.(viol_met_nfkb{k}) = [violin.(viol_met_nfkb{k}), ID(i).metrics.(viol_met_nfkb{k})(ID(i).metrics.responder_index_nfkb == 1 & ID(i).metrics.responder_index_ktr == 1)];
            end
        end
       end   
end
...
switch p.Results.FilterResponders 
    ...
    case 'nfkb'
       for i = 1:n
        for k = 1:numel(viol_met_ktr)
            if i==1 
            violin.(viol_met_ktr{k}) = {ID(i).metrics.(viol_met_ktr{k})(ID(i).metrics.responder_index_nfkb == 1)};
            else
            violin.(viol_met_ktr{k}) = [violin.(viol_met_ktr{k}), ID(i).metrics.(viol_met_ktr{k})(ID(i).metrics.responder_index_nfkb == 1)];
            end
        end
       end
    case {'ktr', 'respective'}
       for i = 1:n
        for k = 1:numel(viol_met_ktr)
            if i==1 
            violin.(viol_met_ktr{k}) = {ID(i).metrics.(viol_met_ktr{k})(ID(i).metrics.responder_index_ktr == 1)};
            else
            violin.(viol_met_ktr{k}) = [violin.(viol_met_ktr{k}), ID(i).metrics.(viol_met_ktr{k})(ID(i).metrics.responder_index_ktr == 1)];
            end
        end
       end
    case 'both'
       for i = 1:n
        for k = 1:numel(viol_met_ktr)
            if i==1 
            violin.(viol_met_ktr{k}) = {ID(i).metrics.(viol_met_ktr{k})(ID(i).metrics.responder_index_nfkb == 1 & ID(i).metrics.responder_index_ktr == 1 )};
            else
            violin.(viol_met_ktr{k}) = [violin.(viol_met_ktr{k}), ID(i).metrics.(viol_met_ktr{k})(ID(i).metrics.responder_index_nfkb == 1 & ID(i).metrics.responder_index_ktr == 1)];
            end
        end
       end    
end
